import streamlit as st
import yfinance as yf
import pandas as pd
import json, os, hashlib, requests
import plotly.express as px

# --- 1. 後端資料核心 ---
F = "data.json"

# 從 Streamlit Secrets 讀取金鑰，確保不被 GitHub 偵測
if "GEMINI_KEY" in st.secrets:
    STABLE_KEY = st.secrets["GEMINI_KEY"]
else:
    st.warning("🔑 請在 Streamlit Cloud 的 Secrets 設定 GEMINI_KEY")
    STABLE_KEY = ""

def ask_gemini(prompt):
    """2026 終極連線邏輯：自動切換路徑避開 404"""
    if not STABLE_KEY: return "❌ 未設定 API Key"
    
    # 按照成功率排列的路徑
    urls = [
        f"https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key={STABLE_KEY}",
        f"https://generativelanguage.googleapis.com/v1/models/gemini-1.5-flash:generateContent?key={STABLE_KEY}"
    ]
    
    refined_prompt = f"你是一位專業股票顧問。請分析以下個股或問題，並給出具體的短期漲跌預估分析與操作建議：\n{prompt}"
    payload = {"contents": [{"parts": [{"text": refined_prompt}]}]}
    headers = {'Content-Type': 'application/json'}
    
    last_err = ""
    for url in urls:
        try:
            response = requests.post(url, headers=headers, json=payload, timeout=15)
            if response.status_code == 200:
                return response.json()['candidates'][0]['content']['parts'][0]['text']
            else:
                last_err = response.json().get('error', {}).get('message', '未知錯誤')
        except: continue
    return f"❌ AI 連線失敗：{last_err}"

def hsh(p): return hashlib.sha256(p.encode()).hexdigest()
def lod():
    if not os.path.exists(F): return {}
    try:
        with open(F, "r", encoding="utf-8") as f: return json.load(f)
    except: return {}
def sav(d):
    with open(F, "w", encoding="utf-8") as f: json.dump(d, f, indent=2)

# --- 2. 漲跌計算邏輯 ---
def calc_limit(price, is_tw=True, direction="up"):
    """計算台股 10% 漲跌停價"""
    change = 1.1 if direction == "up" else 0.9
    raw = price * change
    return round(raw, 2)

# --- 3. 頁面配置與樣式 ---
st.set_page_config(page_title="家族投資戰情室", layout="wide")
st.markdown("""
<style>
    .stMetric { background-color: #f8fafc; padding: 15px; border-radius: 10px; border: 1px solid #e2e8f0; }
    [data-testid="stSidebar"] { background-color: #f1f5f9; }
</style>
""", unsafe_allow_html=True)

if 'db' not in st.session_state: st.session_state.db = lod()
u = st.session_state.get('u')

# --- 4. 登入系統 (落實密碼保護) ---
if not u:
    st.markdown("<h1 style='text-align: center; color: #1e3a8a;'>🛡️ 家族投資安全系統</h1>", unsafe_allow_html=True)
    _, c2, _ = st.columns([1, 1.2, 1])
    with c2:
        uid = st.text_input("👤 帳號")
        upw = st.text_input("🔑 密碼", type="password")
        if st.button("🚀 進入系統", use_container_width=True):
            db = lod()
            if uid and upw:
                ph = hsh(upw)
                # 記憶功能：新用戶設定密碼，老用戶驗證
                if uid not in db: 
                    db[uid] = {"p": ph, "s": []}
                    sav(db)
                if db[uid]["p"] == ph: 
                    st.session_state.u = uid
                    st.session_state.db = db
                    st.rerun()
                else: 
                    st.error("密碼錯誤，請重新輸入。")
    st.stop()

# --- 5. 導覽選單 ---
st.sidebar.markdown(f"### 👤 使用者: **{u}**")
m = st.sidebar.radio("功能導覽", ["📈 資產儀表板", "🤖 AI 投資助手", "🧮 攤平計算機"])
if st.sidebar.button("🔒 安全登出"):
    st.session_state.u = None
    st.rerun()

# --- 6. AI 助手 (預估漲跌分析) ---
if m == "🤖 AI 投資助手":
    st.title("🤖 家族 AI 顧問")
    st.info("💡 提示：輸入代碼（如 2330.TW）即可獲取 AI 的漲跌預估報告。")
    p = st.chat_input("輸入投資問題或股票代碼...")
    if p:
        with st.chat_message("user"): st.write(p)
        with st.spinner("AI 顧問正在分析市場數據..."):
            ans = ask_gemini(p)
            with st.chat_message("assistant"): st.write(ans)

# --- 7. 資產儀表板 (含漲跌停監控) ---
elif m == "📈 資產儀表板":
    st.title("💎 家族資產戰情室")
    try:
        ex_rate = round(yf.Ticker("USDTWD=X").history(period="1d")["Close"].values[-1], 2)
    except:
        ex_rate = 32.5
    
    sk = st.session_state.db[u].get("s", [])
    if sk:
        res = []
        with st.spinner('同步市場報價中...'):
            for i in sk:
                sym = i.get("t", "").strip().upper()
                try:
                    tk = yf.Ticker(sym)
                    df_h = tk.history(period="5d")
                    curr = df_h["Close"].iloc[-1]
                    prev = df_h["Close"].iloc[-2]
                    is_tw = ".TW" in sym or ".TWO" in sym
                    rate = ex_rate if not is_tw else 1.0
                    
                    mv = round(curr * rate * i.get("q", 0))
                    cost = round(i.get("p", 0) * rate * i.get("q", 0))
                    pf = int(mv - cost)
                    
                    res.append({
                        "名稱": i.get("n", ""), "代碼": sym,
                        "昨日收盤": round(prev, 2), "今日現價": round(curr, 2),
                        "預估漲停": calc_limit(prev, is_tw, "up"),
                        "預估跌停": calc_limit(prev, is_tw, "down"),
                        "持股數": i.get("q", 0), "市值": mv, "損益": pf
                    })
                except: continue
        
        if res:
            df = pd.DataFrame(res)
            c1, c2, c3 = st.columns(3)
            c1.metric("💰 總市值 (TWD)", f"{df['市值'].sum():,} 元")
            c2.metric("📊 總盈虧", f"{df['損益'].sum():,} 元", delta=f"{df['損益'].sum():,}")
            c3.metric("💵 美金匯率", f"{ex_rate}")
            
            st.subheader("🔥 漲跌停即時監控表")
            # 損益顏色顯示 (正數綠/負數紅)
            def color_df(val):
                color = '#4ade80' if val >= 0 else '#f87171'
                return f'color: {color}; font-weight: bold'
            
            st.dataframe(df.style.map(color_df, subset=['損益']).format("{:,}", subset=['市值', '損益']), use_container_width=True)
            
            # 圓餅圖與分析
            col_l, col_r = st.columns(2)
            with col_l:
                st.plotly_chart(px.pie(df, values='市值', names='名稱', hole=0.4, title="資產配置分佈"), use_container_width=True)
            with col_r:
                if st.button("🔮 讓 AI 診斷目前持股漲跌", use_container_width=True):
                    names = ", ".join([f"{x['名稱']}({x['代碼']})" for x in res])
                    with st.spinner("AI 顧問分析中..."):
                        report = ask_gemini(f"我的持股：{names}。請分別給予簡短漲跌預估。")
                        st.success("AI 持股診斷報告：")
                        st.write(report)

    with st.expander("🛠️ 持股管理倉儲"):
        with st.form("add_stock"):
            ca, cb, cc, cd = st.columns(4)
            n = ca.text_input("股票名稱")
            t = cb.text_input("代碼 (例: 2330.TW)")
            p = cc.number_input("平均買進成本", value=0.0)
            q = cd.number_input("持有股數", value=0.0)
            if st.form_submit_button("➕ 確認新增"):
                if n and t:
                    db = lod(); db[u]["s"].append({"n":n, "t":t.upper(), "p":p, "q":q})
                    sav(db); st.session_state.db=db; st.rerun()

# --- 8. 攤平計算機 ---
elif m == "🧮 攤平計算機":
    st.title("🧮 成本攤平精算")
    c1, c2 = st.columns(2)
    p1 = c1.number_input("原成本價", value=100.0)
    q1 = c1.number_input("原持股數", value=1000.0)
    p2 = c2.number_input("加碼價格", value=90.0)
    q2 = c2.number_input("加碼股數", value=1000.0)
    
    if (q1 + q2) > 0:
        avg = ((p1 * q1) + (p2 * q2)) / (q1 + q2)
        st.markdown("---")
        st.metric("💡 攤平後預估均價", f"{round(avg, 2)} 元")
